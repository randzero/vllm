...
from vllm.transformers_utils.configs.step1 import Step1Config
...
class StepAttention(nn.Module):
    ...
    def forward(
        ...
        del positions  # positional info handled by alibi
...
class Step1ForCausalLM(nn.Module, SupportsPP):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        super().__init__()
        config = vllm_config.model_config.hf_config
        if not isinstance(config, Step1Config):
            raise TypeError(
                "Step1ForCausalLM requires Step1Config with model_type 'step1'."
            )
        quant_config = vllm_config.quant_config
